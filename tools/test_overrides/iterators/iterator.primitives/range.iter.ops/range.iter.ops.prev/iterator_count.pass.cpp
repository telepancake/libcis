// transferred+adapted from libc++ by tools/transfer.py (slug=iterators_iterator_primitives_range_iter_ops_range_iter_ops_prev_iterator_count).
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_iterators_iterator_primitives_range_iter_ops_range_iter_ops_prev_iterator_count { // libcis
template <typename It>
constexpr void check(int* first, std::iter_difference_t<It> n, int* expected) {
  It it(first);
  auto result = std::ranges::prev(std::move(it), n); // libcis: gcc-10 rejects constrained placeholder with dependent constraint arg here
...
} // libcis_ns_iterators_iterator_primitives_range_iter_ops_range_iter_ops_prev_iterator_count (libcis)
