// transferred+adapted from libc++ by tools/transfer.py (slug=containers_sequences_vector_vector_modifiers_destroy_elements).
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_containers_sequences_vector_vector_modifiers_destroy_elements { // libcis
...
int main() {
  ...
  static_assert(test());
#endif
  return 0; // libcis: upstream main() relies on main's implicit return 0,
            // but the transfer wraps it as a namespaced non-main function,
            // where falling off the end is UB (returned garbage at -O0).
}
} // libcis_ns_containers_sequences_vector_vector_modifiers_destroy_elements (libcis)
