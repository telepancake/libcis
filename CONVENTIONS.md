...
string bloat, the missing C++20 constexpr) walked straight past their handful
of asserts. The ground truth is the ~6650 real libc++ tests under test/std/
(generated by tools/transfer.py; gitignored).

To gate a change to header X, run its ported tests PER FILE:
    python3 tools/run_files.py <subtree>        # e.g. containers/sequences/vector
This compiles+runs each test file on its own against libcis, so one file's
compile error can't blank its neighbours (consolidation cascades; per-file does
not). It prints the FAILURE LIST (the work queue) and a binary verdict, exit 0
iff CLEAN.

State is binary, not a percentage: the library is correct iff EVERY test that
is not *justifiably excluded* passes. "85% pass" and "9% pass" both mean broken.
Do not report conformance as a ratio. Each failing test is either a bug to fix
or a candidate to move -- with a written justification (genuinely impossible on
gcc-10.2 / -fno-exceptions / -fno-rtti, or deprecated-and-out-of-scope) -- into
the exclusion set. The work is to drive the failure list to empty.
