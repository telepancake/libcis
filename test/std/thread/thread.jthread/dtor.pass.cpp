// AST-transferred from libc++ by tools/transfer.py (slug=thread_thread_jthread_dtor).
// main -> test_thread_thread_jthread_dtor; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
void test_thread_thread_jthread_dtor() {
  // !joinable()
  {
    std::jthread jt;
    assert(!jt.joinable());
