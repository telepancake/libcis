// AST-transferred from libc++ by tools/transfer.py (slug=depr_depr_c_headers_ctype_h).
// main -> test_depr_depr_c_headers_ctype_h; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_depr_depr_c_headers_ctype_h { // libcis: isolate file-scope helpers
...
} using namespace libcis_ns_depr_depr_c_headers_ctype_h; // libcis


#include <cassert>

void test_depr_depr_c_headers_ctype_h() {
  assert(isalnum('a'));
  assert(isalpha('a'));
  assert(isblank(' '));
  assert(!iscntrl(' '));
