// AST-transferred from libc++ by tools/transfer.py (slug=depr_depr_c_headers_setjmp_h).
...
namespace libcis_ns_depr_depr_c_headers_setjmp_h { // libcis: isolate file-scope helpers
jmp_buf jb;
ASSERT_SAME_TYPE(void, decltype(longjmp(jb, 0)));

void f() { setjmp(jb); }
} using namespace libcis_ns_depr_depr_c_headers_setjmp_h; // libcis
