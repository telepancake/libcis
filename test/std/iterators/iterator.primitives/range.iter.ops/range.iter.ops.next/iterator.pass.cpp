// transferred+adapted from libc++ by tools/transfer.py (slug=iterators_iterator_primitives_range_iter_ops_range_iter_ops_next_iterator).
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_iterators_iterator_primitives_range_iter_ops_range_iter_ops_next_iterator_ab11c6fb { // libcis
template <class It>
constexpr void check(int* first, int* expected) {
  It it(first);
  auto result = std::ranges::next(std::move(it)); // libcis: gcc-10 rejects constrained placeholder with dependent constraint arg here
...
} // libcis_ns_iterators_iterator_primitives_range_iter_ops_range_iter_ops_next_iterator_ab11c6fb (libcis)
