// AST-transferred from libc++ by tools/transfer.py (slug=numerics_numeric_ops_numeric_ops_midpoint_midpoint_pointer).
// main -> test_numerics_numeric_ops_numeric_ops_midpoint_midpoint_pointer; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
constexpr bool test_pointer() {
  ...
  assert(check<T>(array, 0, 0, 0));
  assert(check<T>(array, 1, 1, 1));
  assert(check<T>(array, 0, 9, 4));
  assert(check<T>(array, 0, 10, 5));
  assert(check<T>(array, 0, 11, 5));
  assert(check<T>(array, 9, 0, 5));
  assert(check<T>(array, 10, 0, 5));
  assert(check<T>(array, 11, 0, 6));
  assert(check<T>(array, 0, 18, 9));
  assert(check<T>(array, 2, 12, 7));
