// AST-transferred from libc++ by tools/transfer.py (slug=utilities_charconv_charconv_syn_chars_format).
// main -> test_utilities_charconv_charconv_syn_chars_format; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_utilities_charconv_charconv_syn_chars_format { // libcis: isolate file-scope helpers
constexpr bool test() {
...
static_assert(std::is_same<std::chars_format&, decltype(x |= x)>::value, "");
static_assert(std::is_same<std::chars_format&, decltype(x ^= x)>::value, "");
} using namespace libcis_ns_utilities_charconv_charconv_syn_chars_format; // libcis


void test_utilities_charconv_charconv_syn_chars_format() {
  assert(test());
  static_assert(test(), "");
