// AST-transferred from libc++ by tools/transfer.py (slug=utilities_utility_utility_inplace_inplace).
// main -> test_utilities_utility_utility_inplace_inplace; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_utilities_utility_utility_inplace_inplace { // libcis: isolate file-scope helpers
template <class Tp, class Up>
constexpr bool check_tag(Up) {
    return std::is_same<Tp, std::decay_t<Tp>>::value
        && std::is_same<Tp, Up>::value;
}
} using namespace libcis_ns_utilities_utility_utility_inplace_inplace; // libcis


void test_utilities_utility_utility_inplace_inplace() {
    // test in_place_t
    {
