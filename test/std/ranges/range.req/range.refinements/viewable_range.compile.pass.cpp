// AST-transferred from libc++ by tools/transfer.py (slug=ranges_range_req_range_refinements_viewable_range).
...
namespace libcis_ns_ranges_range_req_range_refinements_viewable_range { // libcis: isolate file-scope helpers
struct T1 { };
...
static_assert( std::ranges::viewable_range<Holder<Incomplete>* const(&)[10]>);
static_assert(!std::ranges::viewable_range<Holder<Incomplete>* const(&&)[10]>);
} using namespace libcis_ns_ranges_range_req_range_refinements_viewable_range; // libcis
