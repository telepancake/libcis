// AST-transferred from libc++ by tools/transfer.py (slug=ranges_range_access_ssize).
// main -> test_ranges_range_access_ssize; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_ranges_range_access_ssize { // libcis: isolate file-scope helpers
...
static_assert(!std::is_invocable_v<RangeSSizeT, Holder<Incomplete>*&>);
} using namespace libcis_ns_ranges_range_access_ssize; // libcis


void test_ranges_range_access_ssize() {
  test();
  static_assert(test());
