// AST-transferred from libc++ by tools/transfer.py (slug=atomics_atomics_ref_wait).
// main -> test_atomics_atomics_ref_wait; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_atomics_atomics_ref_wait { // libcis: isolate file-scope helpers
template <typename T>
struct TestWait {
  void operator()() const {
    {
...
} using namespace libcis_ns_atomics_atomics_ref_wait; // libcis


void test_atomics_atomics_ref_wait() {
