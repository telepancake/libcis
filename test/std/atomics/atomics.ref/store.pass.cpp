// AST-transferred from libc++ by tools/transfer.py (slug=atomics_atomics_ref_store).
// main -> test_atomics_atomics_ref_store; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_atomics_atomics_ref_store { // libcis: isolate file-scope helpers
template <typename T>
struct TestStore {
  void operator()() const {
...
} using namespace libcis_ns_atomics_atomics_ref_store; // libcis


void test_atomics_atomics_ref_store() {
