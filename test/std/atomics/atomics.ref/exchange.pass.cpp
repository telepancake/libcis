// AST-transferred from libc++ by tools/transfer.py (slug=atomics_atomics_ref_exchange).
// main -> test_atomics_atomics_ref_exchange; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_atomics_atomics_ref_exchange { // libcis: isolate file-scope helpers
template <typename T>
struct TestExchange {
  void operator()() const {
    {
...
} using namespace libcis_ns_atomics_atomics_ref_exchange; // libcis


void test_atomics_atomics_ref_exchange() {
