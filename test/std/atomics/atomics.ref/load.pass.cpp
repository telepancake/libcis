// AST-transferred from libc++ by tools/transfer.py (slug=atomics_atomics_ref_load).
// main -> test_atomics_atomics_ref_load; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_atomics_atomics_ref_load { // libcis: isolate file-scope helpers
template <typename T>
struct TestLoad {
  void operator()() const {
...
} using namespace libcis_ns_atomics_atomics_ref_load; // libcis


void test_atomics_atomics_ref_load() {
