// AST-transferred from libc++ by tools/transfer.py (slug=language_support_support_runtime_ctime_timespec).
...
namespace libcis_ns_language_support_support_runtime_ctime_timespec { // libcis: isolate file-scope helpers
std::timespec tmspec = {};
static_assert(std::is_same<decltype(std::timespec_get(&tmspec, 0)), int>::value, "");
} using namespace libcis_ns_language_support_support_runtime_ctime_timespec; // libcis
