// AST-transferred from libc++ by tools/transfer.py (slug=containers_container_adaptors_priority_queue_priqueue_cons_deduct).
// main -> test_containers_container_adaptors_priority_queue_priqueue_cons_deduct; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_containers_container_adaptors_priority_queue_priqueue_cons_deduct { // libcis: isolate file-scope helpers
struct A {};
} using namespace libcis_ns_containers_container_adaptors_priority_queue_priqueue_cons_deduct; // libcis


void test_containers_container_adaptors_priority_queue_priqueue_cons_deduct() {
  ...
  // - an allocator and a container that uses a different allocator.
  {
    ...
    struct OutputIter {
      ...
      const OutputIter& operator*() const { return; }
      const OutputIter& operator++() { return; }
      OutputIter operator++(int) const { return; }
    };
