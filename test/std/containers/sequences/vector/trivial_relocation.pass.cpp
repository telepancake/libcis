// AST-transferred from libc++ by tools/transfer.py (slug=containers_sequences_vector_trivial_relocation).
// main -> test_containers_sequences_vector_trivial_relocation; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_containers_sequences_vector_trivial_relocation { // libcis: isolate file-scope helpers
struct Tracker {
...
} using namespace libcis_ns_containers_sequences_vector_trivial_relocation; // libcis


void test_containers_sequences_vector_trivial_relocation() {
  tests();
#if TEST_STD_VER >= 20
  static_assert(tests());
#endif
