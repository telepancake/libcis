// AST-transferred from libc++ by tools/transfer.py (slug=containers_sequences_vector_vector_erasure_erase_if).
// main -> test_containers_sequences_vector_vector_erasure_erase_if; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_containers_sequences_vector_vector_erasure_erase_if { // libcis: isolate file-scope helpers
...
} using namespace libcis_ns_containers_sequences_vector_vector_erasure_erase_if; // libcis


void test_containers_sequences_vector_vector_erasure_erase_if() {
  tests();
#if TEST_STD_VER > 17
  static_assert(tests());
#endif
