// AST-transferred from libc++ by tools/transfer.py (slug=containers_sequences_vector_access).
// main -> test_containers_sequences_vector_access; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_containers_sequences_vector_access { // libcis: isolate file-scope helpers
template <class C>
...
} using namespace libcis_ns_containers_sequences_vector_access; // libcis


void test_containers_sequences_vector_access() {
  tests();
#if TEST_STD_VER > 17
  static_assert(tests());
#endif
