// AST-transferred from libc++ by tools/transfer.py (slug=containers_sequences_array_iterators).
// main -> test_containers_sequences_array_iterators; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_containers_sequences_array_iterators { // libcis: isolate file-scope helpers
struct NoDefault {
...
} using namespace libcis_ns_containers_sequences_array_iterators; // libcis


void test_containers_sequences_array_iterators() {
  tests();
#if TEST_STD_VER >= 17
  static_assert(tests(), "");
#endif
