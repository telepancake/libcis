// AST-transferred from libc++ by tools/transfer.py (slug=containers_views_mdspan_mdspan_swap).
// main -> test_containers_views_mdspan_mdspan_swap; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
namespace libcis_ns_containers_views_mdspan_mdspan_swap { // libcis: isolate file-scope helpers
template <class MDS>
...
} using namespace libcis_ns_containers_views_mdspan_mdspan_swap; // libcis


void test_containers_views_mdspan_mdspan_swap() {
  test();
  static_assert(test());
  return;
}
