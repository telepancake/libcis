// AST-transferred from libc++ by tools/transfer.py (slug=containers_unord_unord_multimap_unord_multimap_modifiers_erase_range).
// main -> test_containers_unord_unord_multimap_unord_multimap_modifiers_erase_range; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
void test_containers_unord_unord_multimap_unord_multimap_modifiers_erase_range() {
  {
  ...
  { // Make sure that we're properly updating the bucket list when starting within a bucket
    struct MyHash {
      size_t operator()(size_t v) const { return; }
    };
