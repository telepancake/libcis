// AST-transferred from libc++ by tools/transfer.py (slug=containers_unord_unord_multiset_eq).
// main -> test_containers_unord_unord_multiset_eq; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
void test_containers_unord_unord_multiset_eq() {
  {
  ...
  // Make sure we behave properly when a custom key predicate is provided.
  {
    ...
    struct HashModuloOddness {
      std::size_t operator()(int x) const { return; }
    };
    // A very poor hash
    struct CompareModuloOddness {
      bool operator()(int x, int y) const { return; }
    };
