// AST-transferred from libc++ by tools/transfer.py (slug=strings_string_conversions_stod).
// main -> test_strings_string_conversions_stod; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
void test_strings_string_conversions_stod() {
  // char
  {
    assert(std::stod("0") == 0);
    assert(std::stod("-0") == 0);
