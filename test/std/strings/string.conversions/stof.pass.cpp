// AST-transferred from libc++ by tools/transfer.py (slug=strings_string_conversions_stof).
// main -> test_strings_string_conversions_stof; file-scope helpers isolated in anon namespace.
//===----------------------------------------------------------------------===//
//
...
void test_strings_string_conversions_stof() {
  assert(std::stof("0") == 0);
  assert(std::stof("-0") == 0);
